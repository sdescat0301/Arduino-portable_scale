...
float Weight = 0.0;    //存储计算得到的质量#include <U8glib.h>
#include <Wire.h>
...
void setup()
{
    ...
    base_voltage = (float)analogRead(A7)*5.00/4096;  //读数

}

void loop() 
{
    int16_t adc = analogRead(A7);    //读取挂上重物时力传感器的信号
    Serial.println(adc);
    Voltage = (float)adc * 5.00/4096;    //计算挂上重物之后的电压值
    true_voltage = Voltage - base_voltage;    //计算由于挂上重物而增加的电压值
    Weight = true_voltage * 80 * 500 / 390;    //计算质量
...
void setup()
{
    Serial.begin(9600);    //开始串口通信，设置波特率为9600
    ads.begin();    //开始读数
    ads.setGain(GAIN_TWOTHIRDS);    // 设置增益为2/3x，输入电压的范围为6.144V，实际输入0-5V，输出数字为0-26667
